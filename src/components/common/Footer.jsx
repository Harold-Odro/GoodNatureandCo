...
export function Footer({ variant = 'floral' }) {
  ...
  if (variant === 'minimal') {
    return (
      <footer className="bg-linen-100 py-8">
        <div className="container-custom">
          <div className="text-center">
            <p className="text-charcoal-700 text-sm">
              © {currentYear} {SITE_INFO.name}. {SITE_INFO.location}.
            </p>
          </div>
        </div>
      </footer>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 mb-8">
          ...
          <div>
            ...
            <ul className="space-y-2">
              <li>
                <Link
                  to={ROUTES.floralHome}
                  className="text-charcoal-700 text-sm hover:text-sage-600 transition-colors"
                >
                  Floral Artistry
                </Link>
              </li>
          ...
          {/* Contact Info */}
          <div>
            <h3 className="font-display text-lg text-charcoal-800 mb-4">Get in Touch</h3>
            <ul className="space-y-2 text-sm text-charcoal-700">
              <li>
                <a href={`tel:${CONTACT_INFO.phone.replace(/\D/g, '')}`} className="hover:text-sage-600 transition-colors">
                  {CONTACT_INFO.phone}
                </a>
              </li>
              <li>
                <a href={`mailto:${CONTACT_INFO.email}`} className="hover:text-sage-600 transition-colors">
                  {CONTACT_INFO.email}
                </a>
              </li>
              <li className="pt-2">
                <div className="flex space-x-4">
                  ...
                  <a
                    href={CONTACT_INFO.pinterestUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-charcoal-700 hover:text-sage-600 transition-colors"
            ...
            <p className="mt-4 text-xs text-charcoal-600">
              Service Area: {SITE_INFO.serviceArea}
            </p>
          </div>
        </div>

        {/* Bottom Bar */}
        <div className="border-t border-linen-300 pt-8 text-center">
